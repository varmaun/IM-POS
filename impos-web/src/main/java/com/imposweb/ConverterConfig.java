...
/**
 * This Class register all the custom converters to the spring conversion
 * service
 *
 * @author Jay
 *
...
public class ConverterConfig {
	private final static Logger LOGGER = LoggerFactory
			.getLogger(ConverterConfig.class);
	...
	@PostConstruct
	public void init() {
		GenericConversionService service = beanFactory.getBean(ImposWebApplication.CONVERSION_SERVICE_BEANNAME,
				GenericConversionService.class);
		Collection<Converter> converters = BeanFactoryUtils.beansOfTypeIncludingAncestors(beanFactory, Converter.class)
				.values();
		...
		if (!converters.isEmpty()) {
			...
			for (Converter c : converters) {
				...
				try {
					...
					throw new UnsupportedOperationException("Unable to execute MethodInvoker for 'getRequiredTypeInfo'",
							e);
				}
				LOGGER.debug("Registered Converter [converter={}, source={}, target={}]",
						new Object[] { c, genericTypes[0], genericTypes[1] });
			}
		}
